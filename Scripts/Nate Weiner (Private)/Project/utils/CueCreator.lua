...
---Suffix is inserted after the cue ID portion (before " - ") so that
---"CB-m08 - Title" with suffix "_v1a" becomes "CB-m08_v1a - Title.rpp"
---@param regionName string The region name
---@param suffix string? Optional suffix (e.g., "_v1a")
---@return string fileName The RPP filename
function CueCreator.createCueFileName(regionName, suffix)
    ...
    if dashPos and sfx ~= "" then
        return sanitized:sub(1, dashPos - 1) .. sfx .. sanitized:sub(dashPos) .. ".rpp"
    end
    return sanitized .. sfx .. ".rpp"
end
...
local function createSingleCue(vidProj, region, settings, spotNotes, smptesyncLine, timemodeFields)
    ...
    local vidProjOffset = NW.TimeUtils.getProjectOffset(vidProj)
    local fps = getProjectFps(vidProj)
    ...
    local fileName = CueCreator.createCueFileName(cueId, settings.filenameSuffix)
    ...
    -- ========================================================================
    -- INSERT SUBPROJECT INTO VIDEO PROJECT
    -- ========================================================================
...
function CueCreator.initProcessing(vidProj, videoProjectPath, regions, settings, onProgress, onComplete)
    ...
    local smptesyncLine, timemodeFields = extractFrameRateLines(videoProjectPath)
...
function CueCreator.classifyRegions(proj, regions, cuesTrackName, altCuesTrackName, filenameSuffix)
    local existingNames = {}
    ...
    for _, region in ipairs(regions) do
        local expectedName = sanitizeFolderName(region.name) .. (filenameSuffix or "")
...
function CueCreator.findMissingRegions(proj, regions, cuesTrackName, altCuesTrackName, filenameSuffix)
    local classified = CueCreator.classifyRegions(proj, regions, cuesTrackName, altCuesTrackName, filenameSuffix)
