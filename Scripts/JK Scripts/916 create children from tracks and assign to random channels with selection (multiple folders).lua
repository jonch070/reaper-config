-- Random Channel Routing from Tracks (Multiple Folders) - 9.1.6 With Channel Selection
-- Takes all selected tracks and:
--   * prompts for channels to include and exclude
--   * supports channel names (L, R, C, LFE, Lss, Rss, Lrs, Rrs, Lw, Rw, Ltf, Rtf, Ltm, Rtm, Ltr/Ltb, Rtr/Rtb)
...
-- 9.1.6 SMPTE/Dolby channel mapping
...
local CHANNEL_NAMES_SORTED = {
  ...
  ["lw"] = 9,      -- Lw (Left Wide)
  ["rw"] = 10,     -- Rw (Right Wide)
  ["flt"] = 11,    -- Ltf (Left Top Front, sorted: flt)
  ["frt"] = 12,    -- Rtf (Right Top Front, sorted: frt)
  ["lmt"] = 13,    -- Ltm (Left Top Middle, sorted: lmt)
  ["mrt"] = 14,    -- Rtm (Right Top Middle, sorted: mrt)
  ["lrt"] = 15,    -- Ltr (Left Top Rear, sorted: lrt)
  ["rrt"] = 16,    -- Rtr (Right Top Rear, sorted: rrt)
  ["blt"] = 15,    -- Ltb/Lbt (Left Top Back = Left Top Rear, sorted: blt)
  ["brt"] = 16,    -- Rtb/Rbt (Right Top Back = Right Top Rear, sorted: brt)
}
...
local CHANNEL_NAMES_EXACT = {
  ...
  ["subwoofer"] = 4,
  ["lfe"] = 4,
  ["wide"] = 9,    -- ambiguous but default to left wide
  ["leftwide"] = 9,
  ["rightwide"] = 10,
}
...
local retval, user_input = reaper.GetUserInputs(
  "9.1.6 Channel Selection",
  3,
  "Include (e.g. 1-16 or L, Lss-Rrs, Ltf):,Exclude (e.g. LFE or 4, Lw-Rw):,Fill all channels before repeating? (y/n):,extrawidth=120",
  "1-16,4,y"
)
...
reaper.Undo_EndBlock("Random channel routing 9.1.6 (multiple folders)", -1)
