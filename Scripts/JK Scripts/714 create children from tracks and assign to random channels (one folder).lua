-- Random Channel Routing from Tracks (Single Parent)
-- Takes all selected tracks and:
--   * uses the first selected track as parent folder
--   * ensures parent has at least 12 channels
--   * makes remaining selected tracks children of the parent
--   * routes each child: SRC mono ch1 -> random mono parent channel (1..12, skip 4)
-- Finally selects parent and all children.

local MIN_PARENT_CHANS = 12

-- Utility: make list 1..12 excluding 4
...
reaper.Undo_EndBlock("Random channel routing from tracks (single parent, 12ch)", -1)
