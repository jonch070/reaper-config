-- JK_RoomTone_Insert_0.5s.lua
-- Inserts "Clean silence_0.5 Seconds.mp3" at the current edit cursor
...
local ROOM_TONE_FILE = "Clean silence_0.5 Seconds"   -- extension resolved automatically
...
local function applyFades(track, insert_pos, room_tone_path, xfade_sec)
    -- Locate the newly inserted item by matching source filename near insert_pos
    local basename = room_tone_path:match("[/\\]([^/\\]+)$") or room_tone_path
    local new_item = nil
    ...
    if not new_item then
        -- Fallback: closest item to insert_pos
    ...
    -- Also fade-in the item that immediately follows (shifted right by ripple)
...
local function main()
    ...
    -- Insert at edit cursor on selected track (respects ripple edit setting)
    reaper.InsertMedia(filepath, 0)

    if CROSSFADE_MS > 0 then
        applyFades(track, insert_pos, filepath, CROSSFADE_MS / 1000)
    end
