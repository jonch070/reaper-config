--[[
@description jk_Set CRX snap offset
  @version 0.8
  @author Jonathan Kawchuk
@about
  For each selected CRX item, sets D_SNAPOFFSET so the "corrected word"
  aligns to a project marker when used with
  "Align selected items to markers inside time selection".

  Mode: auto only (driven by pickups CSV + whisper TSV from the pipeline).
  ...
  Steps before running this script:
    1. Split the CRX file into N items (one per pickup, in order)
    2. Run the pipeline to generate:
    ...
    3. Select all CRX items, run this script
...
local function resolve_inputs(first_item)
  ...
  if not cached_project then
    ...
    local ok, path = reaper.GetUserInputs(
      "CRX snap offset — project folder", 1,
...
local function main()
  ...
  local prompt = string.format(
    ...
    n_items, n_pickups)
  if reaper.ShowMessageBox(prompt, "CRX snap offset confirmation", 1) ~= 1 then return end
  ...
  reaper.Undo_EndBlock("Set CRX snap offsets", -1)
  ...
  reaper.ShowConsoleMsg(string.format(
    "CRX snap offset — %d/%d set  |  %d EXACT  %d FAIL\n", exact, total_attempted, exact, fail_count))
  if #skipped > 0 then
