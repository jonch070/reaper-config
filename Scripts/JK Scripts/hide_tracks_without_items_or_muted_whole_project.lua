...
-- A third run hides again, and so on.
--
...
local EXT_SECTION = "JK_HideTracksWithoutItemsOrMuted_WholeProject"
...
local function try_restore()
  ...
  reaper.Undo_EndBlock("Restore track visibility (whole project)", -1)
  return true
end

if try_restore() then return end

local function has_items(track)
...
reaper.TrackList_AdjustWindows(false)
reaper.UpdateArrange()
reaper.Undo_EndBlock("Hide other tracks (whole project)", -1)
