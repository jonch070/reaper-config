...
--[[     if Settings.Erase == true then
    ...
    for i_midi = 1, #ListMidi do
        ...
        for idx_note = 0, notecnt-1 do
            ...
            if vel > Settings.VelocityRange.Max or vel < Settings.VelocityRange.Min then 
                goto continue 
            end
            ----
            ...
            --  Copy 
            local snap_offset = 0
            if Settings and Settings.UseSnapOffset then
                snap_offset = reaper.GetMediaItemInfo_Value(list_sequence[list_idx], "D_SNAPOFFSET")
            end
            local paste_track = reaper.GetMediaItemTrack(list_sequence[list_idx])
            local pasted_item = CopyMediaItemToTrack(list_sequence[list_idx], paste_track, time - snap_offset )
        ...
        for i,v in pairs(Groups) do -- Loop Tabs
            ...
            if open then -- Inside Each Tab
                ...
                if reaper.ImGui_TreeNode(ctx, 'Trim', reaper.ImGui_TreeNodeFlags_DefaultOpen()) then
                    ...
                    if reaper.ImGui_Checkbox(ctx, 'Use Snap Offset',Groups[i].Settings.UseSnapOffset) then
                        Groups[i].Settings.UseSnapOffset = not Groups[i].Settings.UseSnapOffset
                    end
                    if Settings.Tips then ToolTip("Place items at snap offset position relative to MIDI note") end
