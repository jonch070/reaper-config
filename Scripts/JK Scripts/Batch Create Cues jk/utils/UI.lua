-- UI module for Batch Create Cues (jk fork)
-- Handles ImGui settings dialog with ExtState persistence
--
-- JK modification: version tag can optionally be placed at the end of the
-- filename instead of after the cue ID. Uses its own ExtState section so
-- settings stay independent from Nate's original script.
...
local EXTSTATE_SECTION = "JK_BatchCreateCues"
...
---@field filenameSuffix string Version tag appended to the filename (e.g., " - v01.00.00")
---@field versionTagAtEnd boolean Place the version tag at the end of the filename instead of after the cue ID
---@field insertItems boolean Insert cues as subproject items into the session (false = standalone cue files only)
---@field csvSource boolean Create cues from SpotNotes CSV rows (IN/OUT timecodes) instead of session regions
...
local settings = {
    ...
    cuesTrackName = "Cues",
    altCuesTrackName = "",
    filenameSuffix = " - v01.00.00",
    versionTagAtEnd = true,
    insertItems = true,
    csvSource = false,
}
...
local function loadSettings()
    ...
    settings.filenameSuffix = getString("filenameSuffix", " - v01.00.00")
    settings.versionTagAtEnd = getString("versionTagAtEnd", "1") == "1"
    settings.insertItems = getString("insertItems", "1") == "1"
    settings.csvSource = getString("csvSource", "0") == "1"
end
...
local function saveSettings()
    ...
    setString("versionTagAtEnd", settings.versionTagAtEnd and "1" or "0")
    setString("insertItems", settings.insertItems and "1" or "0")
    setString("csvSource", settings.csvSource and "1" or "0")
end
...
local function getExampleFilename()
    if #regionData.classified > 0 then
        return CueCreator.createCueFileName(regionData.classified[1].region.name, settings.filenameSuffix, settings.versionTagAtEnd)
    end
    if settings.versionTagAtEnd then
        return "HGG - 1m01 - Title" .. (settings.filenameSuffix or "") .. ".rpp"
    end
    return "HGG - 1m01" .. (settings.filenameSuffix or "") .. " - Title.rpp"
end
...
local function drawSettingsForm()
    ...
    -- JK addition: drive cue creation from the CSV itself (IN/OUT timecodes)
    local rvCsv
    rvCsv, settings.csvSource = ImGui.ImGui_Checkbox(ctx, "Create cues from CSV rows (uses IN/OUT timecodes)##csvsrc", settings.csvSource)
    if ImGui.ImGui_IsItemHovered(ctx) then
        ImGui.ImGui_SetTooltip(ctx,
            "On: each CSV row becomes a cue named 'CueID - Title', timed by\n" ..
            "its IN/OUT timecode columns. No video project or regions needed;\n" ..
            "cues are created as standalone files only.\n" ..
            "Off: regions in the session define the cues (CSV only annotates).")
    end
    ...
    ImGui.ImGui_SameLine(ctx)
    ImGui.ImGui_SetNextItemWidth(ctx, 130)
    ...
    -- JK addition: option to place the version tag at the end of the filename
    local rvCheckbox
    rvCheckbox, settings.versionTagAtEnd = ImGui.ImGui_Checkbox(ctx, "Place at end of file name##tagpos", settings.versionTagAtEnd)
    if ImGui.ImGui_IsItemHovered(ctx) then
        ImGui.ImGui_SetTooltip(ctx,
            "Off: HGG - 1m01_v1a - Title.rpp\n" ..
            "On:  HGG - 1m01 - Title - v01.01.01.rpp")
    end

    -- JK addition: standalone cue files (no subproject items on the timeline)
    rvCheckbox, settings.insertItems = ImGui.ImGui_Checkbox(ctx, "Insert cue items into session##insitems", settings.insertItems)
    if ImGui.ImGui_IsItemHovered(ctx) then
        ImGui.ImGui_SetTooltip(ctx,
            "On: cues are inserted as subproject items (default).\n" ..
            "Off: cue .rpp files are only created in the output folder\n" ..
            "(standalone sessions; existing-cue detection checks disk).")
    end
...
local function updateRegionData()
    ...
    local regions = getRegionsCallback(settings)

    if classifyRegionsCallback then
        local opts = {
            filesOnly = (not settings.insertItems) or settings.csvSource,
            outputFolder = settings.outputFolder,
        }
        local classified = classifyRegionsCallback(regions, settings.cuesTrackName, settings.altCuesTrackName, settings.filenameSuffix, settings.versionTagAtEnd, opts)
...
local function loop()
    ...
    local visible, open = ImGui.ImGui_Begin(ctx, "Batch Create Cues from Regions (jk)", true)
...
---   - classifyRegions: function(regions, cuesTrackName, altCuesTrackName, filenameSuffix, versionTagAtEnd) Returns ClassifiedRegion[]
function UI.run(vidProj, vidProjectPath, options)
    ...
    ctx = ImGui.ImGui_CreateContext("Batch Create Cues (jk)")
