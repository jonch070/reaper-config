...
function getPositionAfterPPQOffset(initialPosition, ppqOffset)

	local initialPositionPPQ = reaper.MIDI_GetPPQPosFromProjTime(activeTake(), initialPosition)
	local newPositionPPQ = initialPositionPPQ + ppqOffset
	return reaper.MIDI_GetProjTimeFromPPQPos(activeTake(), newPositionPPQ)
end
...
startUndoBlock()
	...
	for i = 1, 240 do
		...
		local lengthOfAttackNote = i
		local attackNoteStartPosition = getPositionAfterPPQOffset(cursorPosition, -lengthOfAttackNote)
		...
		local spacerNoteEndPosition = getPositionAfterPPQOffset(attackNoteEndPosition + numberOfSixteenthNotesForLeadTone * lengthOfSixteenthNote() + 2*lengthOfQuarterNote(), -(lengthOfAttackNote + 1))
