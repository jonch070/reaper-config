...
_gfxw, _gfxh =
  table.unpack(
  _desired_sizes[reaper.GetExtState(sTitle, 'showHelp') == 'false' and 1 or 2]
)
...
function addRandPalette(pal, curpal)
  ...
  for i = 1, #curpal do
    ...
    if a[wadj] > 128 then
      a[wadj] = math.max(a[wadj] - adj, 0)
    else
      a[wadj] = math.min(a[wadj] + adj, 255)
    end
    pal[#pal + 1] = a
  end
  for i = #curpal, 2, -1 do
...
elseif os:match('win') then
  ...
  gfx.setfont(1, 'Roboto', 10)
  gfx.setfont(2, 'Roboto', 10)
  gfx.setfont(3, 'Roboto', 11)
  gfx.setfont(4, 'Roboto', 15)
  gfx.setfont(5, 'Roboto', 12)
  gfx.setfont(11, 'Roboto', 19)
  gfx.setfont(12, 'Roboto', 21)
  gfx.setfont(13, 'Roboto', 21)
  gfx.setfont(14, 'Roboto', 29)
  gfx.setfont(15, 'Roboto', 23)
end

if reaper.LocalizeString ~= nil then
  translate = function(s)
    return reaper.LocalizeString(s or '---', 'Default_6.0_Theme_Adjuster')
  end
else
  translate = function(s)
    return s or '---'
  ...
  for s in str:gmatch('([^%s-/]*[-/]* ?)') do
    ...
    if length > w then
      ...
      while length > w do
        ...
        while wlen > 0 do
          local sstr =
            string.format('%s%s', measure_s:sub(1, wlen), wlen > 1 and '-' or '')
...
function loadImage(idx, name)
  ...
  if gfx.loadimg(idx, str .. name) == -1 then
    reaper.ShowConsoleMsg('image ' .. name .. ' not found')
  end
end
    ...
    if parent.has_children_outside ~= 1 then
      if o.has_children_outside == 1 then
        ...
        if o.x ~= nil and o.y ~= nil and parent.w ~= nil and parent.h ~= nil then
          if
            o.x < 0 or o.y < 0 or o.x + (o.w or 1) > parent.w or
              o.y + (o.h or 1) > parent.h
           then
            parent.has_children_outside = 1
          end
        end
...
function Button:new(parent, o)
  self.__index = self
  o.x, o.y, self.w, self.h, self.border =
    o.x or 0,
    o.y or 0,
    o.w or 30,
    o.h or 30,
...
function Spinner:new(parent, o)
  ...
  if spinStyles[spinStyle].title ~= false then
    local topBar =
      ...
      o,
      {
        x = self.buttonW / 2,
        y = 2,
        w = self.w - self.buttonW,
        h = spinStyles[spinStyle].title.h,
        color = spinStyles[spinStyle].label.col,
        interactive = false
      }
    )
  end
  if spinStyles[spinStyle].readout ~= false then
    local bottomBar =
      ...
      {
        x = self.buttonW / 2,
        y = spinStyles[spinStyle].readout.y,
        w = self.w - self.buttonW,
        h = spinStyles[spinStyle].readout.h,
        color = spinStyles[spinStyle].readout.col,
        interactive = false
      }
    )
  end
  if o.spinStyle == 'image' then
    local ir =
      ...
      o,
      {
        x = self.buttonW,
        y = spinStyles[spinStyle].readout.y,
        w = self.w - (2 * self.buttonW),
        h = spinStyles[spinStyle].readout.h,
        border = '',
        valsImage = o.valsImage,
        action = o.action,
        param = {'editPage'}
      }
    )
      ...
      local r =
        ...
        o,
        {
          ...
          text = {
            ...
            align = 5,
            val = o.value,
            col = spinStyles[spinStyle].readout.strCol,
            style = 2
          },
          action = o.action,
          param = {o.param},
          valsTable = o.valsTable
        }
      )
    end
  end
  ...
  local l =
    ...
    hitBox,
    {
      ...
      w = self.buttonW,
      h = self.h,
      img = spinStyles[spinStyle].buttonLimage,
      imgType = 3,
      action = o.action,
      param = {o.param, -1},
      helpR = o.helpR,
  ...
  local r =
    ...
    hitBox,
    {
      x = self.w - self.buttonW,
      y = 2,
      w = self.buttonW,
      h = self.h,
      img = spinStyles[spinStyle].buttonRimage,
      imgType = 3,
      action = o.action,
      param = {o.param, 1},
      helpR = o.helpR,
  ...
  if o.title ~= nil and spinStyles[spinStyle].title ~= false then
    Element:new(
      o,
      {
        ...
        param = {o.param}, -- label
        text = {
          str = (o.title or 'LABEL STR'),
          align = 5,
          col = spinStyles[spinStyle].label.strCol
        }
      }
    )
  end
...
spinStyles = {
  light = {
    buttonLimage = 'button_left',
    buttonRimage = 'button_right',
    title = {h = 13},
  ...
  for i, v in ipairs(o.valsTable.columns) do
    ...
    Element:new(
      o,
      {
        ...
        w = 80,
        h = 25,
        text = {str = v.text.str, style = 1, align = 1, col = thisCol}
      }
    ) --column titles
  end
  for i = 1, #o.valsTable.rows do
    ParamRow:new(o, o.valsTable, i)
  end
...
function ParamRow:new(parent, valsTable, rowIdx)
  ...
  local row =
    Element:new(parent, {x = 0, y = rowIdx * 25 + 5, w = 453, h = 25, color = rowBgCol})
  local titleW = 114
  ...
  Element:new(
    row,
    {
      ...
      h = 25,
      text = {
        str = valsTable.rows[rowIdx].text.str,
        style = 1,
        align = 6,
        col = {129, 137, 137}
      }
  ...
  for i, v in ipairs(valsTable.columns) do
    Button:new(
      row,
      {
        ...
        imgType = 3,
        action = doFlagParam,
        param = {valsTable.rows[rowIdx].param, v.visFlag}
      }
    )
  end
  adoptChild(parent, o)
  ...
  for i = 1, 10 do
    local p =
      ...
      o,
      {
        ...
        w = o.cellW,
        h = o.h,
        img = o.img or 'color_apply',
        imgType = 3,
        action = o.action
      }
    ) -- used to be x=2 to make the dividers
  end
  adoptChild(parent, o)
  setmetatable(o, self)
  return o
...
function Swatch:new(parent, o)
  ...
  local label =
    ...
    o,
    {
      ...
      w = palStrW,
      h = 17,
      text = {str = palStr, style = 2, align = 9},
      color = {38, 38, 38}
    }
  )
  adoptChild(parent, o)
...
function Fader:doParamGet()
  local lx = self.x
  local tmp, title, value, defValue, min, max =
    reaper.ThemeLayout_GetParameter(self.param)
  if max > min then
...
function paramIdxGet(param)
  if paramsIdx == nil then
    reaper.ShowConsoleMsg('paramsIdx is nil\n')
  end
  local panel = param and string.sub(param, 0, (string.find(param, '%_') - 1))
  if
    param == 'tcp_indent' or param == 'tcp_control_align' or param == 'mcp_indent' or
      param == 'tcp_LabelMeasure' or
      panel == 'envcp' or
      panel == 'trans' or
      panel == 'glb'
   then --params which act on ALL layouts
...
function paramToVal(param, v)
  ...
  if param <= -1001 and param >= -1003 then
    val, suffix = v / 256, ''
  end
  if param == -1004 then
    val, suffix = math.floor(v / 2.56 + .5), ' %'
  end
  if param == -1005 then
    val, suffix = math.floor(v * 0.9375 - 180 + .5), ' °'
  end
  return val, suffix
end
...
function valToParam(param, v)
  ...
  if param == -1004 then
    val = math.floor(v * 2.56 + .5)
  end
  if param == -1005 then
    val = math.floor((v + 180) / 0.9375 + .5)
  end
  return val
end
...
function Element:doParamGet()
  if self.visible ~= false then
    paramGetChildren(self.children)
  end
end
    ...
    if self.valsTable ~= nil then
      if self.action == nil then -- then you're just a palette
        ...
        if value <= #self.valsTable then
          ...
          self.text.str = 'ERR ' .. p .. ' ' .. value
        end
      end
    elseif self.action == doPageSpin then
      self.imgValueFrame = editPage - 1
...
function paramSet(param)
  ...
  if type(p) ~= 'number' then
    p = paramIdxGet(p) or 0
  end
  local name, desc, value, defvalue, minvalue, maxvalue =
    reaper.ThemeLayout_GetParameter(p)
  newValue = value + v
...
function Button:doUpdateState()
  ...
  if self.action == doActiveLayout then
    ...
    if self.param[1] == 'mcp' then
      p = 'P_MCP_LAYOUT'
    end
    if self.param[2] == activeLayout[self.param[1]] then
      a = '_on'
    end --you are the button for the active layout

    self.drawImg = nil
    if a ~= nil then
      self.drawImg = self.img .. a
    end
  end
  if self.children ~= nil then
    for i, v in ipairs(self.children) do
...
function anySelected(self) -- called by an object's updateState value, and uses its getParam values.
  if self.text ~= nil and self.text.colFalse ~= nil and needReaperStateUpdate == 1 then
    ...
    if self.getParam[1] == 'mcp' then
      p = 'P_MCP_LAYOUT'
    end --( NOTE :: param[1] tcp or mcp, param[2] A,B or C.)
  ...
  if needReaperStateUpdate == 1 then
    ...
    if self.imgFalse ~= nil then
      if noneSelected == true then
        self.img = self.imgTrue
      else
        self.img = self.imgFalse
      end
    end
  end
end
  ...
  for i, v in ipairs(reaperDpi) do
    ...
    if reaperDpi[v].old == nil or reaperDpi[v].old ~= reaperDpi[v].new then
      if reaperDpi[v].new > 1.34 then
        p = {2, 3, 4}
        if reaperDpi[v].new > 1.74 then
          p = {1, 2, 3}
        end
      end
      for i = 1, 3 do
        if apply[v][i] == nil then
          apply[v][i] = {}
        end
...
function measureTrackNames(trackCount)
  ...
  for i = 0, trackCount do
    local tmp, trackName =
      reaper.GetSetMediaTrackInfo_String(reaper.GetTrack(0, i), 'P_NAME', '', false)
...
function doHelpVis(visible)
  if visible == nil then
    if reaper.GetExtState(sTitle, 'showHelp') == 'false' then
      visible = false
    else
      visible = true
    end
...
function themeCheck()
  ...
  if theme ~= oldTheme or theme == nil then
    ...
    if theme ~= 'defaultV6' or theme_version < 1 then --theme_version catch for later changes
      ...
      if gfx.measurestr(_theme.text.str) < 160 then
        _theme.w = 160
      else
        _theme.w = gfx.measurestr(_theme.text.str)
      end
      _wrongTheme:onSize()
      redraw = 1
    else
...
function switchTheme()
  ...
  if (reaper.file_exists(str .. '/Default_6.0_unpacked.ReaperTheme') == true) then
    ...
    if (reaper.file_exists(str .. '/Default_6.0.ReaperThemeZip') == true) then
      openTheme = reaper.OpenColorThemeFile(str .. '/Default_6.0.ReaperThemeZip')
    else
      reaper.ShowConsoleMsg('Default 6.0 theme not found')
    end
  end
  indexParams()
  redraw = 1
...
function doActivePage()
  if editPage < 1 or editPage > 6 then
    editPage = 1
  end
...
function doPageSpin(param)
  ...
  if _undockedRoot.visible == true then
    limit = 5
  end
  if editPage >= limit and val == 1 then
    editPage = 1
  else
    if editPage == 1 and val == -1 then
      editPage = limit
    else
      editPage = editPage + val
    end
  end
  doActivePage()
  needReaperStateUpdate = 1
...
function doActiveLayout(param)
  function isLayoutName(n)
    if n ~= nil and (n == 'A' or n == 'B' or n == 'C') then
      return n
    end
  end
  if param ~= nil then
    ...
    for i = 0, reaper.CountTracks(0) - 1 do
      ...
      if reaper.IsTrackSelected(tr) == true then
        reaper.GetSetMediaTrackInfo_String(
          tr,
          p,
          param[2] .. tostring(activeLayout[param[1]]),
          true
        )
      end
    end
...
function doFader(self, dX)
  if self.userEntry == true then --< the fader's readout
    local name, desc, value, defvalue, minvalue, maxvalue =
      reaper.ThemeLayout_GetParameter(self.param[1])
    ...
    local r, v =
      ...
      desc,
      1,
      vValMin .. vValMinSuffix .. ' to ' .. vValMax .. vValMinSuffix,
      self.text.str
    )
    local val = tonumber(v:match('[-]?[%d.,]+'))
    if r ~= false and val ~= nil then
      local tmp, tmp, tmp, tmp, minvalue, maxvalue =
        reaper.ThemeLayout_GetParameter(self.param[1])
...
function Element:onSize()
  ...
  if self.children ~= nil and self.visible ~= false and self.crop ~= true then
    for i, v in ipairs(self.children) do
      local bx, by = 0, 0
      if v.border == 'xy' or v.border == 'x' then
        bx = globalBorderX
      end
      if v.border == 'xy' or v.border == 'y' then
        by = globalBorderY
      end

      local prevElX, prevElY, prevElW, prevElH = 0, 0, 0, 0
      if i > 1 then --there is a previous child
        prevElX, prevElY, prevElW, prevElH =
          self.children[i - 1].drawx,
          self.children[i - 1].drawy,
          self.children[i - 1].drawW,
          self.children[i - 1].drawH
        if self.children[i - 1].visible == false then
          prevElW = 0
        end
      end
      ...
      if v.flexW ~= nil then
        if v.flexW == 'fill' then
          v.drawW =
            self.drawx + (self.drawW or self.w) -
            ((prevElX or self.drawx) + (prevElW or 0)) +
            (v.w or 0)
        else
          v.drawW = (v.w or 0) + ((self.drawW or self.w) * (v.flexW:sub(1, -2) / 100))
        end
      else
        v.drawW = v.w
      end
      ...
      if v.flexH ~= nil then
        v.drawH = (v.h or 0) + ((self.drawH or self.h) * (v.flexH:sub(1, -2) / 100))
      else
        v.drawH = v.h
      end
...
function Element:draw()
  ...
  if self.visible ~= false and crop ~= true then
      ...
      if self.imgType ~= nil and iDw ~= nil and self.imgType == 3 then
        ...
        if thisW ~= iDw / 3 then -- width stretching needed
          ...
          gfx.blit(
            ...
            1,
            0,
            (iDw / 3) * (self.imgFrame or 0) + (iDw / 3 - pad),
            0,
            pad,
            iDh,
            thisX + thisW - pad,
    ...
    if self.text ~= nil then
      ...
      local tx, tw =
        thisX + (drawScale * textPadding),
        thisW - 2 * (drawScale * textPadding)
      text(
        self.text.str,
        tx,
        thisY,
        ...
        self.text.align,
        self.text.col,
        txtScaleOffs .. (self.text.style or 1),
        self.text.lineSpacing,
        self.text.vCenter,
        self.text.wrap
      )
    end
...
function Element:mouseDown(x, y)
end
function Element:mouseUp(x, y)
end
function Element:doubleClick()
end
...
function FaderBg:mouseDown(x, y)
  local tmp, tmp, value, tmp, minvalue, maxvalue =
    reaper.ThemeLayout_GetParameter(self.param)
  local v =
    minvalue +
    math.floor(((x / drawScale - self.drawx - 10) / 432) * (maxvalue - minvalue))
...
function Element:hitTest(x, y)
  local thisX, thisY, thisW, thisH =
    self.drawx or self.x,
    self.drawy or self.y,
    self.drawW or self.w or 0,
    self.drawH or self.h
  local xS, yS = x / drawScale, y / drawScale
  if self.visible ~= false then
    local inrect =
      xS >= thisX and yS >= thisY and xS < thisX + thisW and yS < thisY + thisH
...
helpL_layout =
  'These settings are automatically saved to your REAPER install, and ' ..
  'will be used whenever you use this theme.'
helpL_customCol =
  'Any assigned custom colors will be saved with your ' .. 'project, when it is saved.'
helpL_colDimming =
  'This theme draws custom colors at full strength. Old ' ..
  'projects may appear very bright, dim them here.'
helpL_dock = 'REAPER will remember whether you docked this script, and where.'
helpL_applySize =
  'Layout and scale assignments are part of your REAPER ' ..
...
helpR_colAdj =
  'Adjust how REAPER draws the theme colors.  Mousewheel for fine ' ..
  'adjustment. Double click the fader to reset. Double click the value to enter a new value.'
helpR_resetColAdj =
  'Reset all of these color controls to return the theme to its unaltered state.'
helpR_indent =
  'Amount to indent a panel due to its depth within your project folder ' ..
  'structure.  Value chosen is used by all layouts.'
helpR_control_align =
  'Choose whether to indent the panel controls when ' ..
...
helpR_default =
  "Indicates whether this layout is the 'default' layout. " ..
  'To choose your default use the Options > Layouts menu or the ' ..
    'Screensets/Layouts window.'
helpR_selected =
  'Indicates whether one or more of the selected tracks is using this layout.'
helpR_applySize =
  'Applies this layout to any selected tracks, at this size. ' ..
  'REAPER may already be using a non-100% size, depending on your ' .. 'HiDPI settings.'
helpR_nameSizeEnv =
  "Size of the envelopes' name field.  If set to AUTO then, " ..
  'while the script is running, it will adjust this to fit the ' ..
    'longest envelope name currently in your project.'
helpR_meterScale =
  "Requires 'DO METER EXPANSION' to be ticked below, and " ..
...
helpR_borders =
  'Adds visual separation to your mixer with borders. ' ..
...
help_pref =
  'These buttons set REAPER preferences. Their settings are automatically ' ..
  'saved to your REAPER install.'

helpR_recolProject =
  'Assigns random colors from the palette. Tracks which share ' ..
  'a color will be given the same new color.'
helpR_choosePalette = 'Click to select a palette.'

helpR_emvMatchIndent =
  "Indent with folders, matching the 'Folder Indent " ..
  "Width' setting on the Track Control Panel Page."
help_playRate =
  "If 'Show Play Rate' is on, sets the size of the play rate control. " ..
...
root =
  Element:new(
  nil,
  {x = 0, y = 0, drawx = 0, drawy = 0, w = _gfxw, h = _gfxh, color = {38, 38, 38}}
)
...
_theme =
  Element:new(
  _wrongTheme,
  {x = 32, y = 2, w = 100, h = 15, text = {str = '', align = 4, col = {129, 137, 137}}}
)
Element:new(
  _theme,
  {
    ...
    w = 160,
    h = 15,
    text = {str = 'is not compatible with this script', align = 4, col = {230, 23, 91}}
  }
)
_switchTheme =
  Button:new(
  _wrongTheme,
  {
    ...
    action = switchTheme,
    text = {
      str = 'Switch to the#Default v6 theme',
      align = 5,
      lineSpacing = 9,
      col = {129, 137, 137}
    }
...
_pageGlobal =
  Element:new(
  _dockedRoot,
  {flow = true, title = 'GLOBAL', y = 0, flexW = 'fill', x = 0, w = -16, h = _gfxh}
)
Spinner:new(
  _pageGlobal,
  {
    flow = true,
    spinStyle = 'image',
    valsImage = 'page_titles_small',
    x = 6,
    y = 0,
    w = 133,
    border = '',
    action = doPageSpin,
    readoutParam = {editPage}
  }
)
_gammaUnd =
  Element:new(
...
Fader:new(
  _gammaUndBg,
  {x = 1, y = -4, action = doFader, param = -1000, helpR = helpR_colAdj}
)
_gammaBox =
  Element:new(_pageGlobal, {flow = true, x = 0, y = -6, w = 72, h = 30, border = 'x'})
Element:new(
  _gammaBox,
  {
    x = 0,
    ...
    w = 72,
    h = 11,
    text = {str = 'GAMMA', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Readout:new(
  _gammaBox,
  {
    ...
    moColor = {60, 60, 60},
    param = {-1000},
    text = {str = '', align = 4, col = {129, 137, 137}}
  }
)
...
Element:new(
  _saturationUndBg,
  {x = 226, y = 2, w = 1, h = 13, color = {255, 255, 255, 64}}
) --zero line
Fader:new(
  _saturationUndBg,
  {x = 1, y = -4, action = doFader, param = -1004, helpR = helpR_colAdj}
)
_saturationBox =
  Element:new(_pageGlobal, {flow = true, x = 0, y = -6, w = 72, h = 30, border = 'x'})
Element:new(
  _saturationBox,
  {
    ...
    w = 72,
    h = 11,
    text = {str = 'SATURATION', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Readout:new(
  _saturationBox,
...
_pageTrack =
  Element:new(
  _dockedRoot,
  {flow = true, title = 'TRACK', y = 0, flexW = 'fill', x = 0, w = -16, h = _gfxh}
)
Spinner:new(
  _pageTrack,
  {
...
_applyBoxTcp =
  Element:new(_pageTrack, {flow = true, w = 228, h = 30, color = {51, 51, 51}})
...
_pageMixer =
  Element:new(
  _dockedRoot,
  {flow = true, title = 'MIXER', y = 0, flexW = 'fill', w = -16, h = _gfxh}
)
Spinner:new(
  _pageMixer,
  {
...
_applyBoxMcp =
  Element:new(_pageMixer, {flow = true, w = 228, h = 30, color = {51, 51, 51}})
...
Spinner:new(
  _pageMixer,
  {
    ...
    border = 'x',
    action = paramSet,
    param = 'mcp_meterExpSize',
    valsTable = mcpMeterExpVals
  }
)
Button:new(
  _pageMixer,
...
_pageColors =
  Element:new(
  _dockedRoot,
  {flow = true, title = 'COLORS', x = 0, y = 0, flexW = 'fill', w = -16, h = _gfxh}
)
Spinner:new(
  _pageColors,
  {
...
_palette =
  Palette:new(
  _pageColors,
  {flow = true, border = 'y', w = 318, h = 30, cellW = 30, action = applyCustCol}
)
Spinner:new(
  _pageColors,
  {
    ...
    w = 96,
    title = 'PALETTE',
    action = paletteChoose,
    value = getCurrentPalette
  }
)
Button:new(
  _pageColors,
  {flow = true, img = 'color_apply_all', imgType = 3, border = 'x', action = applyPalette}
)
Button:new(
  _pageColors,
  {
...
_pageEnv =
  Element:new(
  _dockedRoot,
  {flow = true, title = 'ENVELOPE', y = 0, flexW = 'fill', w = -16, h = _gfxh}
)
Spinner:new(
  _pageEnv,
  {
...
Spinner:new(
  _pageEnv,
  {
    ...
    border = 'x',
    action = paramSet,
    param = 'envcp_labelSize',
    valsTable = envcpLabelVals
  }
)
Spinner:new(
  _pageEnv,
...
Button:new(
  ...
  {
    flow = true,
    img = 'match_folder_indent',
    imgType = 3,
    border = 'x',
    action = paramToggle,
    param = 'envcp_folder_indent'
  }
)
...
_pageTrans =
  Element:new(
  _dockedRoot,
  {flow = true, title = 'TRANSPORT', y = 0, flexW = 'fill', w = -16, h = _gfxh}
)
Spinner:new(
  _pageTrans,
  {
...
_applyBoxTrans =
  Element:new(_pageTrans, {flow = true, w = 188, h = 30, color = {51, 51, 51}})
apply.trans = {
  Button:new(
    _applyBoxTrans,
    {
...
Spinner:new(
  _pageTrans,
  {
    ...
    border = 'x',
    action = paramSet,
    param = 'trans_rate_size',
    valsTable = transRateVals
  }
)
Button:new(
  _pageTrans,
...
_pageContainer =
  Element:new(
  _undockedRoot,
  {positionX = 'center', positionY = 'center', x = 0, y = 0, w = 513, h = 679}
)
...
Button:new(
  _pageGlobal_und,
  {
    ...
    imgType = 3,
    action = paramToggle,
    param = 'glb_track_label_color',
    helpR = helpR_trackLabelColor
  }
)
Element:new(
  _pageGlobal_und,
  {
    ...
    w = 165,
    h = 28,
    text = {str = 'Custom color track names', style = 2, align = 4, col = {129, 137, 137}},
    helpR = helpR_trackLabelColor
  }
)

_colAdjBoxStroke =
  Element:new(_pageGlobal_und, {x = 0, y = 88, w = 513, h = 475, color = {51, 51, 51}}) -- stroke
_colAdjBox =
  Element:new(
  _colAdjBoxStroke,
  {x = 1, y = 1, w = 511, h = 473, color = {51, 51, 51}, helpL = helpL_layout}
) -- fill
Element:new(
  _colAdjBox,
  {
    ...
    w = 511,
    h = 11,
    text = {str = 'COLOR CONTROLS', style = 2, align = 5, col = {129, 137, 137}},
    helpR = helpR_trackLabelColor
  }
)
...
Fader:new(
  _highlightbg,
  {x = 1, y = -4, action = doFader, param = -1003, helpR = helpR_colAdj}
)
Fader:new(
  _midtonebg,
  {x = 1, y = -4, action = doFader, param = -1002, helpR = helpR_colAdj}
)
Fader:new(
  _shadowbg,
  {x = 1, y = -4, action = doFader, param = -1001, helpR = helpR_colAdj}
)
Element:new(
  _colAdjBox,
  {
    ...
    w = 72,
    h = 11,
    text = {str = 'HIGHLIGHTS', style = 2, align = 4, col = {129, 137, 137}},
    helpR = helpR_colAdj
  }
)
Readout:new(
...
Element:new(
  _colAdjBox,
  {
    ...
    w = 72,
    h = 11,
    text = {str = 'MIDTONES', style = 2, align = 6, col = {129, 137, 137}},
    helpR = helpR_colAdj
  }
)
Readout:new(
  _colAdjBox,
  {
    ...
    moColor = {40, 40, 40},
    param = {-1002},
    text = {str = '', align = 5, col = {129, 137, 137}},
    helpR = helpR_colAdj
  }
)
Element:new(
  _colAdjBox,
  {
    ...
    w = 72,
    h = 11,
    text = {str = 'SHADOWS', style = 2, align = 6, col = {129, 137, 137}},
    helpR = helpR_colAdj
  }
)
Readout:new(
  _colAdjBox,
  {
    ...
    moColor = {40, 40, 40},
    param = {-1001},
    text = {str = '', align = 6, col = {129, 137, 137}},
    helpR = helpR_colAdj
  }
)
...
Element:new(
  _colAdjBox,
  {
    ...
    w = 72,
    h = 11,
    text = {str = 'TINT', style = 2, align = 4, col = {129, 137, 137}},
    helpR = helpR_colAdj
  }
)
Readout:new(
...
Element:new(
  _colAdjBox,
  {
    ...
    h = 30,
    text = {
      str = 'Also affect project custom colors',
      style = 2,
      align = 4,
      col = {129, 137, 137}
    }
...
Button:new(
  _colAdjBox,
  {
    ...
    img = 'bin',
    imgType = 3,
    action = resetColorControls,
    helpR = helpR_resetColAdj
  }
)
Element:new(
  _colAdjBox,
  {
    ...
    w = 165,
    h = 28,
    text = {str = 'Reset all color controls', style = 2, align = 4, col = {129, 137, 137}},
    helpR = helpR_resetColAdj
  }
)
...
Spinner:new(
  _pageTrack_und,
  {
    ...
    action = paramSet,
    param = 'tcp_control_align',
    valsTable = trackControlAlignVals,
    helpR = helpR_control_align,
    helpL = helpL_layout
  }
)
_layoutTrackStroke =
  Element:new(_pageTrack_und, {x = 0, y = 153, w = 513, h = 486, color = {129, 137, 137}}) -- stroke
...
Readout:new(
  _tcpButLayA,
  {
    ...
    updateState = anySelected,
    getParam = {'tcp', 'A'},
    helpR = helpR_selected
  }
)
Readout:new(
  _tcpButLayB,
...
_applyStroke_Track =
  Element:new(_pageTrack_und, {x = 274, y = 64, w = 239, h = 89, color = {129, 137, 137}}) -- stroke
_applyBox_Track =
  Element:new(
  _applyStroke_Track,
  {x = 1, y = 1, w = 237, h = 89, color = {51, 51, 51}, helpL = helpL_applySize}
) -- fill
apply.und = {
  tcp = {
    Button:new(
...
_layoutTrack =
  Element:new(
  _layoutTrackStroke,
  {x = 1, y = 1, w = 511, h = 484, color = {51, 51, 51}, helpL = helpL_layout}
) -- fill
Spinner:new(
  _layoutTrack,
  {
...
tcpTableVals = {
  img = 'cell_hide',
  columns = {
    {visFlag = 1, text = {str = 'IF MIXER#IS VISIBLE'}},
    {visFlag = 2, text = {str = 'IF TRACK#NOT SELECTED'}},
    {visFlag = 4, text = {str = 'IF TRACK#NOT ARMED'}},
    {visFlag = 8, text = {str = 'ALWAYS#HIDE', col = {204, 69, 114}}}
  },
  rows = {
    {param = 'tcp_Record_Arm', text = {str = 'RECORD ARM'}, img = 'key_recarm'},
    {param = 'tcp_Monitor', text = {str = 'MONITOR'}, img = 'key_mon'},
    {param = 'tcp_Track_Name', text = {str = 'TRACK NAME'}},
    {param = 'tcp_Volume', text = {str = 'VOLUME'}, img = 'key_vol'},
    {param = 'tcp_Routing', text = {str = 'ROUTING'}, img = 'key_route'},
    {param = 'tcp_Effects', text = {str = 'INSERT FX'}, img = 'key_fx'},
    {param = 'tcp_Envelope', text = {str = 'ENVELOPE'}, img = 'key_env'},
    {param = 'tcp_Pan_&_Width', text = {str = 'PAN & WIDTH'}, img = 'key_pan'},
    {param = 'tcp_Record_Mode', text = {str = 'RECORD MODE'}, img = 'key_recmode'},
    {param = 'tcp_Input', text = {str = 'INPUT'}, img = 'key_recinput'},
...
_trackTable =
  ParamTable:new(
  _layoutTrack,
  {x = 29, y = 125, w = 453, h = 330, valsTable = tcpTableVals}
)
...
Spinner:new(
  _pageMixer_und,
  {
    ...
    title = 'ALIGN CONTROLS',
    action = paramSet,
    param = 'mcp_control_align',
    valsTable = controlAlignVals,
    helpR = helpR_control_align,
    helpL = helpL_layout
  }
)
_mixerTopStroke =
  Element:new(_pageMixer_und, {x = 0, y = 153, w = 513, h = 245, color = {129, 137, 137}}) -- stroke
...
_applyStroke_Mixer =
  Element:new(
  _pageMixer_und,
  {x = 274, y = 64, w = 239, h = 89, color = {129, 137, 137}, helpL = helpL_applySize}
) -- stroke
_applyBox_Mixer =
  Element:new(
  _applyStroke_Mixer,
...
_mixerTop =
  Element:new(
  _mixerTopStroke,
  {x = 1, y = 1, w = 511, h = 243, color = {51, 51, 51}, helpL = helpL_layout}
) -- fill
Spinner:new(
  _mixerTop,
  {
...
Spinner:new(
  _mixerTop,
  {
    ...
    y = 29,
    w = 157,
    title = 'METER EXPANSION',
    action = paramSet,
    param = 'mcp_meterExpSize',
    valsTable = dockedMcpMeterExpVals,
    helpR = helpR_meterScale
  }
)
...
mcpTableVals = {
  img = 'cell_tick',
  columns = {
    {visFlag = 1, text = {str = 'IF TRACK#SELECTED'}},
    {visFlag = 2, text = {str = 'IF TRACK#NOT SELECTED'}},
    {visFlag = 4, text = {str = 'IF TRACK#ARMED'}},
    {visFlag = 8, text = {str = 'IF TRACK#NOT ARMED'}}
  },
  rows = {
    {param = 'mcp_Sidebar', text = {str = 'EXTEND WITH SIDEBAR', helpR = helpR_sidebar}},
    {param = 'mcp_Narrow', text = {str = 'NARROW FORM'}},
    {param = 'mcp_Meter_Expansion', text = {str = 'DO METER EXPANSION'}},
    {param = 'mcp_MLabels', text = {str = 'METER READOUT'}},
    {param = 'mcp_Labels', text = {str = 'ELEMENT LABELS'}}
  }
}
_mixerTable =
  ParamTable:new(_mixerTop, {x = 29, y = 84, w = 453, h = 200, valsTable = mcpTableVals})

_mLower =
  Element:new(_pageMixer_und, {x = 0, y = 427, w = 513, h = 212, color = {129, 137, 137}}) -- stroke
_extMixerf =
  Element:new(
  _mLower,
  {x = 1, y = 1, w = 255, h = 210, color = {38, 38, 38}, helpL = help_pref}
) -- fill
Element:new(
  _extMixerf,
  {
    ...
    h = 30,
    text = {
      str = 'EXTENDED MIXER CONTROLS#when size permits',
      style = 2,
      align = 4,
      col = {129, 137, 137}
    }
...
Element:new(
  _extMixerf,
  {
    ...
    w = 172,
    h = 30,
    text = {str = 'Show FX Inserts', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _extMixerf,
...
Element:new(
  _extMixerf,
  {
    ...
    w = 172,
    h = 30,
    text = {str = 'Show FX Parameters', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _extMixerf,
...
Element:new(
  _extMixerf,
  {
    ...
    w = 172,
    h = 30,
    text = {str = 'Show Sends', style = 2, align = 4, col = {129, 137, 137}}
  }
)
...
_parmMixer =
  Element:new(
  _mLower,
  {x = 256, y = 1, w = 256, h = 210, color = {51, 51, 51}, helpL = help_pref}
) -- fill
Button:new(
  _parmMixer,
  {
...
Element:new(
  _parmMixer,
  {
    ...
    h = 30,
    text = {
      str = 'Show Multiple-row Mixer#when size permits',
      style = 2,
      align = 4,
      col = {129, 137, 137}
    }
...
Element:new(
  _parmMixer,
  {
    ...
    w = 172,
    h = 30,
    text = {str = 'Scroll to selected track', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _parmMixer,
...
Element:new(
  _parmMixer,
  {
    ...
    w = 172,
    h = 30,
    text = {str = 'Show Icons', style = 2, align = 4, col = {129, 137, 137}}
  }
)
...
_paletteBoxStroke =
  Element:new(_pageColor_und, {x = 0, y = 0, w = 513, h = 425, color = {129, 137, 137}}) -- stroke
_paletteBox =
  Element:new(
  _paletteBoxStroke,
  {x = 1, y = 1, w = 511, h = 205, color = {51, 51, 51}, helpL = helpL_customCol}
) -- fill
...
Readout:new(
  _paletteBox,
  {
    ...
    w = 263,
    h = 30,
    param = {'scriptVariable', palette.current},
    text = {str = 'REAPER V6', style = 4, align = 5, col = {129, 137, 137}},
    valsTable = undockPaletteNamesVals
  }
)
...
Element:new(
  _paletteBox,
  {
    ...
    h = 30,
    text = {
      str = 'Click a color to apply it to all selected tracks',
      style = 3,
      align = 5,
      col = {129, 137, 137}
    }
...
Element:new(
  _paletteBox,
  {
    ...
    h = 30,
    text = {
      str = 'Recolor project using this palette',
      style = 2,
      align = 4,
      col = {129, 137, 137}
    },
...
_paletteMenuBox =
  Element:new(_paletteBoxStroke, {x = 1, y = 205, w = 511, h = 218, color = {38, 38, 38}}) -- fill
...
Element:new(
  _pageColor_und,
  {
    ...
    h = 30,
    text = {
      str = 'Dim all assigned custom colors',
      style = 2,
      align = 4,
      col = {129, 137, 137}
    },
...
Element:new(
  _pageColor_und,
  {
    ...
    h = 30,
    text = {
      str = 'Dim custom colors on selected tracks',
      style = 2,
      align = 4,
      col = {129, 137, 137}
    },
...
--ENV & TRANSPORT PAGE
_pageEnvTrans_und =
  Element:new(_subPageContainer, {x = 0, y = 40, w = 513, h = 639, helpL = helpL_layout})
apply.und.envcp = {
  Button:new(
    _pageEnvTrans_und,
    {
...
Element:new(
  _pageEnvTrans_und,
  {x = 210, y = 252, w = 94, h = 23, img = 'transport_title'}
)
...
_transPrefsStroke =
  Element:new(
  _pageEnvTrans_und,
  {x = 0, y = 431, w = 513, h = 176, color = {129, 137, 137}}
) -- stroke
_transPrefs =
  Element:new(
  _transPrefsStroke,
  {x = 1, y = 1, w = 511, h = 174, color = {38, 38, 38}, helpL = helpL_pref}
) -- fill
...
Element:new(
  _transPrefs,
  {
    ...
    w = 150,
    h = 30,
    text = {str = 'Center transport', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _transPrefs,
...
Element:new(
  _transPrefs,
  {
    ...
    w = 150,
    h = 30,
    text = {str = 'Show Play Rate', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _transPrefs,
  {x = 29, y = 112, img = 'time_sig', imgType = 3, action = actionToggle, param = 40680}
)
Element:new(
  _transPrefs,
  {
    ...
    w = 150,
    h = 30,
    text = {str = 'Show Time Signature', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _transPrefs,
  {x = 285, y = 69, img = 'next_prev', imgType = 3, action = actionToggle, param = 40868}
)
Element:new(
  _transPrefs,
  {
    ...
    w = 150,
    h = 30,
    text = {str = 'Use Home/End for Markers', style = 2, align = 4, col = {129, 137, 137}}
  }
)
Button:new(
  _transPrefs,
  {x = 285, y = 112, img = 'play_text', imgType = 3, action = actionToggle, param = 40532}
)
Element:new(
  _transPrefs,
  {
    ...
    w = 150,
    h = 30,
    text = {str = 'Show play state as text', style = 2, align = 4, col = {129, 137, 137}}
  }
)
...
function runloop()
  ...
  if
    gfx.mouse_x ~= mouseXold or gfx.mouse_y ~= mouseYold or
      (firstClick ~= nil and last_click_time ~= nil and last_click_time + .25 < now)
   then
    firstClick = nil
  end

  if
    gfx.mouse_x ~= mouseXold or gfx.mouse_y ~= mouseYold or isCap ~= mouseCapOld or
      gfx.mouse_wheel ~= 0
   then
    local wheel_amt = 0
    ...
    if activeMouseElement ~= nil then
      ...
      if isCap == 1 then
        ...
        if firstClick == nil or last_click_time == nil then
          ...
          if
            now < last_click_time + .25 and
              math.abs(
                (x - firstClick[1]) * (x - firstClick[1]) +
                  (y - firstClick[2]) * (y - firstClick[2])
              ) < 4
           then
  ...
  if
    resize == 1 or root.drawW ~= gfx.w * drawScale_inv_mac or
      root.drawH ~= gfx.h * drawScale_inv_mac
   then -- window resized
  ...
  if ctheme_param_needsave ~= nil then
    if
      (gfx.mouse_cap & 1) == 0 and
        (ctheme_param_needsave[2] == nil or now > ctheme_param_needsave[2])
     then
...
function storeTable(title, table, parent)
  for i, v in pairs(table) do
    ...
    if type(v) == 'table' then
      storeTable(title, v, i)
    else
      reaper.SetExtState(sTitle, title .. '.' .. p .. i, v, true)
    end
  end
end
